...
binPointCount = nnz(pointBins==binNo)                                                       //No. of nonzero matrix elements
binEdgeLengths = binCorners(binNo,1:3) - binCorners(binNo,4:6)								//Size / Volume
binDepth = binDepths(binNo)                                                               	//No. Subdivisions
...
newDiv = (binCorners(binNo,1:3) + binCorners(binNo,4:6)) / 2								//Matrix addition, then division
for i = 1:8
	newBinNo = length(binDepths) + 1														//current depth + 1
	binDepths(newBinNo) = binDepths(binNo) + 1												//addnew depth
